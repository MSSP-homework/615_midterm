...
test <- leaflet() %>%
  addTiles() %>%
  addCircleMarkers(noaa_20_21 = BEGIN_LAT, lng = ~Longitude, lat = ~Latitude, color = "blue", radius = 5) %>%
  addCircleMarkers(data = end_data, lng = ~Longitude, lat = ~Latitude, color = "red", radius = 5) %>%
  addPolylines(data = begin_data, lng = ~c(Longitude, end_data$Longitude), lat = ~c(Latitude, end_data$Latitude), color = "blue")
m
